...
class Snake extends SnakeComponent {
  ...
  void checkHeadOverflow() {
    ...
    if (offset.dx > canvasSize!.width) {
      ...
      updateHead(
          position: Offset(canvasSize!.width + snakeWidth / 2 + _shift(),
              head!.position.dy));
      ...
      if (collisionPoint != null) {
        ...
        body!
          ...
          ..add(SnakeJoint(Offset(canvasSize!.width + overflowX, offset.dy),
              head!.direction));
      }
    } else if (offset.dy > canvasSize!.height) {
      final overflowY = offset.dy - canvasSize!.height;
      final last =
          Offset(offset.dx, canvasSize!.height + gridSize() - _shift());
      ...
      updateHead(
          position: Offset(head!.position.dx,
              canvasSize!.height + snakeWidth / 2 + _shift()));
      ...
      if (collisionPoint != null) {
        ...
        body!
          ..add(SnakeJoint(
              Offset(offset.dx, canvasSize!.height), head!.direction))
  ...
  void checkTailOverflow() {
    ...
    if (offset.dx > canvasSize!.width) {
      ...
      updateTail(
          position: Offset(tail!.position.dx,
              canvasSize!.height + snakeWidth / 2 + _shift()));
    }
  }

  void updateHead(
      {SnakeJoint? end, Offset? position, SnakeDirection? direction}) {
    if (end != null) {
      head = end;
      detectCollision();
    } else if (head != null) {
